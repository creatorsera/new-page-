...
/* ── LEFT PANEL — target the main layout block by :has(textarea) ── */
div[data-testid="stHorizontalBlock"]:has(.stTextArea) > div:first-child {
    position: relative;
    background: #ffffff !important;
    border-right: 1px solid #e2e2de !important;
    padding-top: 0 !important;
    overflow: hidden;
}
div[data-testid="stHorizontalBlock"]:has(.stTextArea) > div:first-child::before {
    content: '';
    position: absolute;
...
.pill {
    ...
    border-radius: 99px; padding: 3px 10px; color: #475569;
...
hr { border-color: #f1f5f9 !important; margin: 10px 0 !important; }
...
/* ── BIG START BUTTON ──────────────────────────────── */
.stButton > button[key="btn_s"],
.stButton > button[key="btn_ns"] {
